...
__kernel void calclsetDiffFlags2D(
  CALint rows,
  CALint columns,
  CALint borderSize,
  CALint chunkSize,
...
inline void mergeBuffers(CALint i,
	              __global CALbyte* A,
                __global CALbyte* B){
...
__kernel void calclMergeFlags2D(
  ...
  if(threadID < 2*borderSizeReal){
    ...
    if(threadID < borderSizeReal){
      //chunkNum = threadID/chunkSize;
      mergeBuffers(threadID,mergeflagsBuffer,flagsReal);
    }
    else{
      // chunkNum = ((rows-1)*columns+threadID)/chunkSize;
         mergeBuffers(threadID-borderSizeReal,
