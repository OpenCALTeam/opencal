...
CALCLToolkit2D * calclCreateToolkit2D(struct CALModel2D *model, CALCLcontext context, CALCLprogram program, CALCLdevice device, CALCLOptimization opt) {
...
CALbyte calclSingleStep2D(CALCLToolkit2D* toolkit2d, struct CALModel2D * model, size_t * threadsNum, int dimNum) {
	...
	CALbyte activeCells = toolkit2d->opt == CALCL_OPT_ACTIVE_CELLS;
	int j;
