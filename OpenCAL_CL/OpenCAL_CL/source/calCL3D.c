...
void calclMapperToSubstates3D(struct CALModel3D *model, CALCLSubstateMapper * mapper) {
	...
	size_t elNum = model->columns * model->rows * model->layers;
...
void calclSetKernelStreamCompactionArgs3D(CALCLToolkit3D * toolkit, struct CALModel3D * model) {
	CALint dim = model->rows * model->columns * model->layers;
...
void calclSetKernelsLibArgs3D(CALCLToolkit3D *toolkit, struct CALModel3D * model) {
	...
	clSetKernelArg(toolkit->kernelUpdateSubstate, 2, sizeof(CALint), &model->layers);
...
void calclSetModelParameters3D(CALCLToolkit3D * toolkit3d,struct CALModel3D * model, CALCLkernel * kernel) {
	...
	clSetKernelArg(*kernel, 2, sizeof(CALCLmem), &toolkit3d->bufferLayers);
	...
	int chunk = ceil((double)(model->rows * model->columns*model->layers)/toolkit3d->streamCompactionThreadsNum);
...
CALCLqueue calclCreateQueue3D(CALCLToolkit3D * toolkit, struct CALModel3D * model, CALCLcontext context, CALCLdevice device) {
	...
	while (model->rows * model->columns * model->layers <= toolkit->streamCompactionThreadsNum)
...
CALCLToolkit3D * calclCreateToolkit3D(struct CALModel3D *model, CALCLcontext context, CALCLprogram program,CALCLdevice device,CALCLOptimization opt) {
	...
	if (model->A.flags == NULL) {
		model->A.flags = (CALbyte*) malloc(sizeof(CALbyte) * model->rows * model->columns * model->layers);
		memset(model->A.flags, CAL_FALSE, sizeof(CALbyte) * model->rows * model->columns * model->layers);
	}

	cl_int err;
	int bufferDim = model->columns * model->rows * model->layers;
	...
	calclHandleError(err);
	toolkit->bufferLayers = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(CALint), &model->layers, &err);
	calclHandleError(err);
...
void calclRun3D(CALCLToolkit3D* toolkit3d, struct CALModel3D * model, unsigned maxStep) {
	...
	threadNumMax[2] = model->layers;
	size_t * singleStepThreadNum;
	int dimNum;

	if (toolkit3d->opt == CALCL_NO_OPT) {
...
CALbyte calclSingleStep3D(CALCLToolkit3D* toolkit3d, struct CALModel3D * model, size_t * threadsNum, int dimNum) {
	...
	CALbyte activeCells = toolkit3d->opt == CALCL_OPT_ACTIVE_CELLS;
	int j;
...
void calclFinalizeToolkit3D(CALCLToolkit3D * toolkit) {
	...
	clReleaseMemObject(toolkit->bufferLayers);
