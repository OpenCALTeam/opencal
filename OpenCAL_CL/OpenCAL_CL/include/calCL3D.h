...
typedef struct CALCLToolkit3D {
	CALCLOptimization opt;								//!< Enumeration used for optimization strategies (CALCL_NO_OPT, CALCL_OPT_ACTIVE_CELL).
	...
	CALCLmem bufferLayers;								//!< Opencl buffer used to transfer GPU side the number of CA layers
...
CALCLToolkit3D * calclCreateToolkit3D(struct CALModel3D *model,		//!< Pointer to a CALModel3D
		...
		CALCLOptimization opt										//!< Optimization strategies (CALCL_NO_OPT, CALCL_OPT_ACTIVE_CELL)
		);
