...
void calglDrawDiscreetModel3D(struct CALGLDrawModel3D* calDrawModel) {
	glPushMatrix(); {
		glPushAttrib(GL_LIGHTING_BIT); {
			...
			if(calglAreLightsEnable()&&!calDrawModel->modelLight) {
				calDrawModel->modelLight = calglCreateLightParameter(
					calglGetPositionLight(),
					calglGetAmbientLight(),
					calglGetDiffuseLight(),
			...
			glTranslatef(-(calDrawModel->calModel->rows/2.0f), -(calDrawModel->calModel->columns/2.0f), -(calDrawModel->calModel->slices/2.0f));
