    ...
    void run(opencal::CALModel<2 , opencal::CALVonNeumannNeighborhood<2> , COORD_TYPE>* calModel, std::array<uint,2>& indexes)
    {
        ...
        for (n=1; n<neighboorhoodSize; n++)
            ...
            else
            {
                ...
                ((CALModelUnsafe<2 , opencal::CALVonNeumannNeighborhood<2> , COORD_TYPE>*)calModel)->addActiveCellX(linearIndex, n);
            }
    }
...
class SciddicaT_remove_inactive_cells : public opencal::CALLocalFunction<2 , opencal::CALVonNeumannNeighborhood<2> , COORD_TYPE>
{
private:
    struct SciddicaTSubstates* Q;
    struct SciddicaTParameters* P;
public:
    ...
    void run(opencal::CALModel<2 , opencal::CALVonNeumannNeighborhood<2> , COORD_TYPE>* calModel, std::array<uint,2>& indexes)
    {
        if (Q->h->getElement(indexes)<= this->P->epsilon)
        {
...
SciddicaTModel::SciddicaTModel (std::array<COORD_TYPE,2>& coords): sciddicaT(coords,&neighbor,opencal::calCommon::CAL_SPACE_TOROIDAL, opencal::calCommon::CAL_OPT_ACTIVE_CELLS),
