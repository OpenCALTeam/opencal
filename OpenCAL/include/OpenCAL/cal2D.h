...
struct CALModel2D {
    ...
    struct CALActiveCells2D* A;			//!< Computational Active cells object (naive implementation). if A.actives==NULL no optimization is applied.
    struct CALContiguousLinkedList2D* contiguousLinkedList; //!< Computational Active cells object (optimal implementation). if A.actives==NULL no optimization is applied.
