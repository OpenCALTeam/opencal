...
void display(void)
{
	...
	glTranslatef(-iso->columns/2.0f, iso->rows/2.0f, 0);
	glScalef(1, -1, 1);

	sciddicaTComputeExtremes(iso, Q.value, &_min, &_Max);

	for (i=0; i<iso->rows; i++)
		for (j=0; j<iso->columns; j++)
		{
			s = calGet2Dr(iso,Q.value,i,j);
	...
	glColor3d(0,1,0);
	glRectd(0,0,iso->columns, iso->rows);
...
void simulationRun(void)
{
	...
	if (!again)
	{
		...
		//graphic rendering
		printf("step: %d; \tactive cells: %d\r", isoRun->step, isoRun->ca2D->A.size_current);
		glutPostRedisplay();
		return;
	}
...
void reshape(int w, int h)
{
	...
	glLoadIdentity();
	if (iso->rows > iso->columns) dim = iso->rows * 0.5f; else dim = iso->columns * 0.5f;
