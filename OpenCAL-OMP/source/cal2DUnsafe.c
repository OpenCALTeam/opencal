...
void calAddActiveCellX2D(struct CALModel2D* ca2D, int i, int j, int n)
{
    ...
    if (ca2D->T == CAL_SPACE_FLAT)
    {
        if(ca2D->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            calAddActiveCellNaive2D(ca2D, (i + ca2D->X[n].i), (j + ca2D->X[n].j));
    ...
    {
        if(ca2D->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            calAddActiveCellNaive2D(ca2D, calGetToroidalX(i + ca2D->X[n].i, ca2D->rows), calGetToroidalX(j + ca2D->X[n].j, ca2D->columns));
        else if(ca2D->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS)
